...
public final class OpenProjectFilterHandler
{
    ...
    {
        return mergeIntoLivedataFilters(filters, handleFilterNode(filtersNode), false);
    }

    /**
     * Merges a list of {@link LiveDataQuery.Filter} with filters expressed in the format used by the OpenProject API.
     * The livedata filters take precedence: a property filtered on both sides keeps the livedata operator, the values
     * of the two sides being merged. The values generated by OpenProject are placed before the livedata ones, since
     * some operators, i.e. contains, take only the first value of a filter into account.
     *
     * @param filters the list of {@link LiveDataQuery.Filter} objects representing the base filters
     * @param apiFiltersNode a JsonNode representing filters in the OpenProject API format, i.e.
     *     {@code [{"subject":{"operator":"~","values":["a"]}}]}
     * @return a string compatible with the OpenProject API's expected filter format.
     * @throws ProjectManagementException if an error occurs during parsing or serialization
     * @since 1.2.1
     */
    public static String mergeApiFilters(List<LiveDataQuery.Filter> filters, JsonNode apiFiltersNode)
        throws ProjectManagementException
    {
        return mergeIntoLivedataFilters(filters, handleApiFilterNode(apiFiltersNode), true);
    }

    private static String mergeIntoLivedataFilters(List<LiveDataQuery.Filter> filters,
        Map<String, Object> convertedFiltersFromNode, boolean nodeValuesFirst) throws ProjectManagementException
    {
        ...
        mergeFiltersMaps(mergedFilters, convertedFiltersFromNode, nodeValuesFirst);
    ...
    private static Map<String, Object> handleApiFilterNode(JsonNode apiFilterNode)
    {
        Map<String, Object> convertedFilters = new HashMap<>();

        if (apiFilterNode == null || apiFilterNode.isEmpty()) {
            return convertedFilters;
        }

        for (JsonNode filterNode : apiFilterNode) {
            Iterator<Map.Entry<String, JsonNode>> properties = filterNode.fields();
            ...
            while (properties.hasNext()) {
                ...
                for (JsonNode value : property.getValue().path(VALUES)) {
                    values.add(value.asText());
                }

                Map<String, Object> constraints = new HashMap<>();
                constraints.put(property.getValue().path(OPERATOR).asText(), values);

                convertedFilters.put(property.getKey(), constraints);
            }
        }
        return convertedFilters;
    }
            ...
            if (firstMap.containsKey(entry.getKey())) {
                ...
                secondFilter.forEach((key, value) -> {
                    if (firstFilter.containsKey(key)) {
                        firstFilter.put(key,
                            mergeFilterValues((List<String>) firstFilter.get(key), (List<String>) value,
                                secondMapValuesFirst));
                    } else {
    ...
    private static List<String> mergeFilterValues(List<String> firstValues, List<String> secondValues,
        boolean secondValuesFirst)
    {
        List<String> mergedValues = new ArrayList<>(secondValuesFirst ? secondValues : firstValues);

        for (String value : secondValuesFirst ? firstValues : secondValues) {
